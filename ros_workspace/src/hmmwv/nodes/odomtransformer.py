...
def callback(data, tfOutput):
	bc = tf2_ros.TransformBroadcaster()
	t = geometry_msgs.msg.TransformStamped()

	t.header.stamp = rospy.Time.now()
	t.header.frame_id = "/scanmatcher_odom"
...
if __name__ == "__main__":
	rospy.init_node("tfOutput")
	odomInput = rospy.get_param("~odom_input")
	tfOutput  = rospy.get_param("~tf_output")
	rospy.Subscriber(odomInput, nav_msgs.msg.Odometry, callback, tfOutput)
	rospy.spin()
